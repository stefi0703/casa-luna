...
export default function FacilitiesDetailed({ onClose, t, language, toggleLanguage }) {
  ...
  const handleBackToSite = () => {
    onClose ? onClose() : router.push(prefix("/"));
  };
  ...
  return (
    <Box bg="#F9F8F6" minH="100vh">
      <Navbar
        ...
        scrollToSection={(id) => (window.location.href = prefix(`/#${id}`))}
        onOpenGallery={() => (window.location.href = prefix("/#gallery"))}
      />
                ...
                >
                  ...
                  <SimpleGrid columns={{ base: 1, md: 2 }} gap={4}>
                    <List.Root variant="plain" spacing={2} color={mainTextColor}>
                      {section.items
                        .slice(0, Math.ceil(section.items.length / 2))
                        .map((item, idx) => (
                    ...
                    <List.Root variant="plain" spacing={2} color={mainTextColor}>
                      {section.items
                        .slice(Math.ceil(section.items.length / 2))
                        .map((item, idx) => (
                          <List.Item key={idx}>{item}</List.Item>
                        ))}
                    </List.Root>
              ...
              <SimpleGrid columns={{ base: 1, md: 2 }} gap={10}>
                ...
                <Box>
                  <Heading
                    ...
                    color="gray.800"
                  >
                    <CigaretteOff size={24} color={accentColor} />{" "}
                    {data.sections.smoking.title}
                  </Heading>
                  <List.Root variant="plain" spacing={3} fontSize="md" color="gray.700">
